...
data = None
weights_path = os.path.abspath("./model/resnet18.npy")
...
class Resnet18:
    ...
    def residual_block_first(self, x, channel_equal=False, strides=2, name='unit'): # 需要进行升降维
        with tf.variable_scope(name) as scope:
            if channel_equal:
                if strides == 1:
                    shortcut = tf.identity(x) # 返回本身
                else:
    ...
    def residual_block(self, x, name='unit'): # 直接映射
        with tf.variable_scope(name) as scope:
            shortcut = x
            conv1 = self.conv_layer(x, name=name + '_conv_1') # 卷积+BN
            relu1 = tf.nn.relu(conv1, name=name + 'relu1')
            conv2 = self.conv_layer(relu1, name=name + '_conv_2')
    ...
    def build(self, bgr_input):
        ...
        _, self.res3 = self.residual_block_first(self.res2, name='conv_3_1') # 残差块之前做了池化
        _, self.res4 = self.residual_block(self.res3, 'conv_3_2')

        _, self.res5 = self.residual_block_first(self.res4, name='conv_4_1') # 加入了池化的残差块
        _, self.res6 = self.residual_block(self.res5, 'conv_4_2')

        _, self.res7 = self.residual_block_first(self.res6, name='conv_5_1')
        _, self.res8 = self.residual_block(self.res7, 'conv_5_2') # 没有全连接层
